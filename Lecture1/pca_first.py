...
os.environ.setdefault("TF_CPP_MIN_LOG_LEVEL", "2")  # Report only TF errors by default # only errors
...
def main(args):
    ...
    data = tf.reshape(data, shape=[data.shape[0], data.shape[1] * data.shape[2] * data.shape[3]])
    ...
    cov = cov / tf.cast(tf.shape(data)[0], tf.float32)
    ...
    diagonal_part = tf.linalg.diag_part(cov)
    total_variance = tf.math.reduce_sum(diagonal_part)
    ...
    for i in range(args.iterations):
        ...
        v = tf.linalg.matmul(cov, v)
        s = tf.linalg.norm(v, ord="euclidean")
        v = v / s
